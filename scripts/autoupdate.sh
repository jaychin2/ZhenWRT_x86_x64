...
download_file () {
    ...
    wget https://github.com/jaychin2/ZhenWRT_x86_x64/releases/download/$(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)-Lean$choose/openwrt-x86-64-generic-squashfs-combined-efi.img.gz
    wget https://github.com/jaychin2/ZhenWRT_x86_x64/releases/download/$(date -d "@$(($(busybox date +%s) - 86400*($days-1)))" +%Y.%m.%d)-Lean$choose/sha256sums
    exist_judge
}
#存在判断
exist_judge () {
