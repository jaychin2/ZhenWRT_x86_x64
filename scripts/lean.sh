...
svn co https://github.com/jaychin2/packages/branches/ok/admin/netdata ./feeds/packages/admin/netdata
...
wget -P target/linux/rockchip/patches-5.4 https://raw.githubusercontent.com/jaychin2/package_target/master/107-Add-support-for-off-on-delay-kernel-5.4.patch
# Change default shell to zsh
sed -i 's/\/bin\/ash/\/usr\/bin\/zsh/g' package/base-files/files/etc/passwd

# Modify default IP
sed -i 's/192.168.1.1/192.168.2.1/g' package/base-files/files/bin/config_generate
sed -i '/uci commit system/i\uci set system.@system[0].hostname='ZhenWrt'' package/lean/default-settings/files/zzz-default-settings
sed -i "s/OpenWrt /jaychin2 @ ZhenWrt $(TZ=UTC-8 date "+%Y%m%d") /g" package/lean/default-settings/files/zzz-default-settings
...
echo -e " jaychin2's ZhenWrt built on "$(date +%Y.%m.%d)"\n -----------------------------------------------------" >> package/base-files/files/etc/banner
